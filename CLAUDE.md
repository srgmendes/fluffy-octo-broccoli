...
| **Graphify integration** | repo root (`GRAPHIFY.md`), `.agents/skills/graphify/` | The `graphify` CLI (PyPI `graphifyy`, installed per-machine) that turns folders into knowledge graphs, plus its vendored skill. |
| **Agent skills** | `.agents/skills/`, `.claude/skills/`, `skills-lock.json` | Vendored third-party Claude skills (`find-skills`, `research`, `graphify`) pinned by hash or version. |
...
├── GRAPHIFY.md              # Graphify CLI install + vendored-skill docs
...
├── .agents/skills/          # Vendored skill sources (find-skills, research, graphify)
...
entry); plus `graphify`, whose skill ships inside the `graphifyy` PyPI package
(see `GRAPHIFY.md`). Because graphify's skill is copied out of an installed CLI
rather than checked out from a GitHub `SKILL.md`, it has **no `skills-lock.json`
entry** either — its version is pinned instead by
`.agents/skills/graphify/.graphify_version`. Unlike the other skills, graphify
also needs a per-machine CLI install (`uv tool install graphifyy`) for its
`/graphify` commands to actually run.

When adding or updating a GitHub-vendored skill, update `skills-lock.json`
(including the hash) alongside the files. When adding a local or
package-provided skill, just place `SKILL.md` under `.agents/skills/<name>/` and
create the matching `.claude/skills/<name>` symlink — no lockfile entry. Either
way, keep the `.claude/skills` symlink in place.
