...
## Rules
- **Context Materialization**: If the plan involves the current project (refactoring, testing, learning), you **MUST** use tools (ls/read) first to get project structure/stack and write it into the "Background and Context" section. No "Cloud Planning".
...
## Workflow
1.  **Active Exploration**: If project planning is involved, prioritize using tools to scan project structure.
2.  **Analyze and Extract**: Deeply analyze the user's input in "【...】" to extract the core objective and any hidden context.
3.  **Define Role and Goal**: Conceive the most suitable expert role for the AI to perform the task and define a clear, measurable final goal.
4.  **Inject Project Context**: Integrate gathered project info (language, framework, directories) into the Prompt.
5.  **Plan Key Steps**: Break down the process of completing the task into several key steps, providing clear execution guidance for each.
6.  **Specify Output Requirements**: Define the specific format, style, and constraints that the final output must adhere to.
7.  **Combine and Generate**: Combine all the above elements into a new, structured prompt that conforms to the format requirements below.
